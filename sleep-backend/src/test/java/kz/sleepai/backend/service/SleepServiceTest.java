...
class SleepServiceTest {
    ...
    void getUserHistory_ShouldReturnSessionHistory() {
        ...
        when(sleepSessionRepository.findAllByUser_EmailOrderByStartTimeDesc(
                org.mockito.ArgumentMatchers.eq(TEST_EMAIL),
                any(org.springframework.data.domain.Pageable.class)))
                .thenReturn(Arrays.asList(testSession, session2));

        // Act
        List<SleepSession> result = sleepService.getUserHistory(TEST_EMAIL, 50);
    ...
    void getLatestSession_ShouldReturnLatest() {
        // Arrange
        when(sleepSessionRepository.findFirstByUser_EmailOrderByStartTimeDesc(TEST_EMAIL))
